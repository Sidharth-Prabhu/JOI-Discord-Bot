...
async def db_add_shared_expense(guild_id: str, channel_id: str, creator_id: str, title: str, total_amount: float, split_count: int, due_date: datetime, payment_url: str, is_recurring: bool, mentioned_users: list):
    ...
    try:
        ...
        print(f"Error adding shared expense: {e}")
        return False
    finally:
        if conn and conn.is_connected():
            cursor.close()
...
async def db_update_shared_expense(expense_id: str, title: str, total_amount: float, split_count: int, due_date: datetime, payment_url: str, is_recurring: bool):
    ...
    try:
        ...
        print(f"Error updating shared expense: {e}")
        return False
    finally:
        if conn and conn.is_connected():
            cursor.close()
...
async def db_delete_shared_expense(expense_id: str):
    ...
    try:
        ...
        print(f"Error deleting shared expense: {e}")
        return False
    finally:
        if conn and conn.is_connected():
            cursor.close()
...
# ────────────────────────────────────────────────
# SHARED EXPENSE VIEWS & MODALS
# ────────────────────────────────────────────────

class SharedExpenseModal(ui.Modal):
    def __init__(self, mentioned_users: list, expense_id: str = None, initial_data: dict = None):
        title = "Edit Shared Expense" if expense_id else "Create Shared Expense"
        ...
        self.title_input = ui.TextInput(
            ...
            placeholder="e.g. Netflix Subscription, Pizza Party",
            default=initial_data.get('title', '') if initial_data else '',
            required=True
        )
...
class SharedExpenseRecurringView(ui.View):
    ...
    async def save_expense(self, interaction: discord.Interaction, is_recurring: bool):
        guild_id = str(interaction.guild_id)
        ...
        payment_url = self.data['payment_url']

        if self.expense_id:
            success = await db_update_shared_expense(self.expense_id, title, total_amount, split_count, due_date, payment_url, is_recurring)
            msg = f"✅ Shared expense '**{title}**' updated!"
        else:
            success = await db_add_shared_expense(guild_id, channel_id, creator_id, title, total_amount, split_count, due_date, payment_url, is_recurring, self.mentioned_users)
            msg = f"✅ Shared expense '**{title}**' created! Reminders will start 5 days before {due_date.strftime('%Y-%m-%d')}."

        if success:
            await interaction.response.edit_message(content=msg, view=None)
        else:
            await interaction.response.edit_message(content="Failed to save shared expense. Please try again.", view=None)

    @ui.button(label="Yes, Recurring", style=discord.ButtonStyle.green)
    async def recurring_yes(self, interaction: discord.Interaction, button: ui.Button):
        await self.save_expense(interaction, True)

    @ui.button(label="No, One-time", style=discord.ButtonStyle.grey)
...
class SharedExpenseSelectView(ui.View):
    def __init__(self, expenses: list, action: str):
        ...
        self.select = ui.Select(
            placeholder=f"Select expense to {action}...",
            options=options
        )
        self.select.callback = self.callback
        self.add_item(self.select)
...
class MarkPaidSelectView(ui.View):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        success = await db_mark_user_paid(self.expense_id, user_id)
        if success:
            await interaction.response.edit_message(content=f"✅ Marked <@{user_id}> as PAID.", view=None)
        else:
...
class TriggerExpenseSelectView(ui.View):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if success:
            ...
            await interaction.response.edit_message(content=f"ℹ️ All users have already paid for '**{expense['title']}**'. No reminder sent.", view=None)
...
async def cmd_shared_expense(interaction: discord.Interaction, users: str):
    member_ids = re.findall(r'<@!?(\d+)>', users)
    if not member_ids:
...
async def cmd_edit_shared_expense(interaction: discord.Interaction):
    ...
    class ActionView(ui.View):
        ...
        @ui.button(label="Edit Info", style=discord.ButtonStyle.primary)
        async def edit_btn(self, inter: discord.Interaction, button: ui.Button):
            await inter.response.edit_message(content="Select an expense to edit:", view=SharedExpenseSelectView(self.expenses, "edit"))

        @ui.button(label="Mark User as Paid", style=discord.ButtonStyle.success)
        async def paid_btn(self, inter: discord.Interaction, button: ui.Button):
            await inter.response.edit_message(content="Select an expense to mark user as paid:", view=SharedExpenseSelectView(self.expenses, "mark paid"))

        @ui.button(label="Delete Expense", style=discord.ButtonStyle.danger)
        async def delete_btn(self, inter: discord.Interaction, button: ui.Button):
            await inter.response.edit_message(content="Select an expense to DELETE:", view=SharedExpenseSelectView(self.expenses, "delete"))
...
async def cmd_trigger_shared_expense(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("Select an expense to send a reminder for:", view=TriggerExpenseSelectView(expenses), ephemeral=True)
...
async def expense_reminder_task():
    # Run daily at 9:00 AM IST (3:30 AM UTC) to check for expenses due in <= 5 days
    # Note: bot.guilds might not be fully populated immediately, but on_ready calls it.
    for guild in bot.guilds:
        ...
        for exp in expenses:
            ...
            if days_left <= 5:
                ...
                if channel:
                    await send_expense_reminder(channel, exp)
...
async def cmd_diskspace_adm(interaction: discord.Interaction):
    ...
    try:
        # Use controlled output format (safer parsing)
        command_to_execute = "df -h --output=source,size,used,avail,pcent,target /dev/mmcblk0p2"
        ...
        if result.returncode != 0:
            error_message = (
                f"Failed to get disk space information.\n"
        ...
        # Truncate raw output if needed (Discord embed limit protection)
        max_output_length = 1014
        ...
        embed.add_field(
            name="Raw Output",
            value=f"```bash\n{output}{output_truncated_message}\n```",
            inline=False
        )

        embed.set_footer(text="Disk space info • JOI Bot")

        await interaction.followup.send(embed=embed)

    except FileNotFoundError:
        await interaction.followup.send(
            "The necessary command (`df`) was not found on the server.",
            ephemeral=True
        )
...
@tree.command(name="post-chat", description="Post a message as the bot")
@app_commands.describe(
    message="The message you want the bot to post"
)
async def post_chat(interaction: discord.Interaction, message: str):
    msg_text = message.strip()
    if not msg_text:
        await interaction.response.send_message("Cannot send an empty message.", ephemeral=True)
        return
...
async def on_ready():
    ...
    if not expense_reminder_task.is_running():
        expense_reminder_task.start()
        print("[TASK] Expense reminder task started.")
