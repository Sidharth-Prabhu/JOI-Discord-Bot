...
class CalendarPaginationView(ui.View):
    ...
    async def create_embed(self):
        ...
        try:
            ...
            cursor.execute("SELECT * FROM academic_calendar WHERE event_date BETWEEN %s AND %s ORDER BY event_date ASC", (start_date, end_date))
            ...
            embed = discord.Embed(title=f"📅 Academic Calendar - {month_name} {self.year}", color=0x3498db)
            ...
            if events:
                ...
                embed.add_field(name="Events", value="No events scheduled.", inline=False)
...
class AcademicCalendar(commands.Cog):
    ...
    async def calendar_view(self, interaction: discord.Interaction, month: int = None, year: int = None):
        ...
        view = CalendarPaginationView(self.bot, self.get_db_connection, month, year, self.get_type_emoji)
    ...
    async def upcoming_events(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(title="🚀 Upcoming Academic Events", color=0x2ecc71)
            for e in events:
